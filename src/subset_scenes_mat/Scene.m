classdef Scene < handle
  ...
  methods 
    ...
    function m_scene = Scene(name,focal_pxid, dims, buf, years, cols)
        ...
        set_geo_axes(m_scene.buf-1, dims(1) - buf, dims(2) - buf);
    ...
    function set_elev_mat(m_scene)
      start = m_scene.buf; 
      rend = m_scene.dim(1) - m_scene.buf - 1;
      cend = m_scene.dim(2) - m_scene.buf - 1;
    ...
    function range_01 = adjust_range(m_scene)
      range_01 = m_scene.pearson_mat - min(m_scene.pearson_mat(:));  
      range_01 = range_01 ./ max(m_scene.pearson_mat(:));
    end
    ...
    function elev_fig = elev_fig(m_scene)
      ...
      imshow(m_scene.elev_mat, [], 'Colormap', parula(256));
      title("Elevation Map");
      colorbar;
      cur_height = num2str( round(m_scene.idim(1) / 4) );
      xlabel( append(cur_height, "km") );
